...
```python
genre = Director.objects.get(id=7)
genre.country = 'USA'
genre.save()
```
...
```
get은 반드시 하나, 없거나 많으면 오류를 띄우지만, filter은 반드시 리스트가 결과가 나온다.
```
...
```python
genre = Director.objects.get(id=8)
genre.delete()
(1, {'db.Director': 1})
```